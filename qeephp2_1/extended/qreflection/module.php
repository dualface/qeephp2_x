...
class QReflection_Module
{
    ...
    function controllersName()
    {
        if (is_null($this->_controllers_name))
        {
            ...
            foreach ($files as $file)
            {
                ...
                $ns = isset($names[1]) ? "{$names[0]}::" : '';
                $controller_name = array_pop($names);
                $this->_controllers_name[] = "{$ns}{$controller_name}";
            }
        }
     ...
     * @return boolean
     */
    function hasController($controller_name, $ns = null)
    {
        $udi = strtolower($controller_name);
        $names = explode('::', $controller_name);
        if (!isset($names[1]))
        {
            if ($ns) $udi = strtolower($ns) . '::' . $udi;
        }
     ...
     */
    function generateController($controller_name, $ns = null)
    {
        return $this->controllerGenerator()->generate($controller_name, $ns);
    }
